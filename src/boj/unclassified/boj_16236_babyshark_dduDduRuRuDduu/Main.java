...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int[] currentPrey = bfs(start);

        System.out.println(Arrays.toString(currentPrey));
    }
    // bfs로 상어가 이동한 좌표를 구하기
    static int[] bfs(int[] startCoord){
        Queue<int[]> q = new ArrayDeque<>();
        q.offer(startCoord);

        int min_dist = N*N;
        int[] targetCoord = new int[] {N, N};
        ...
        while (!q.isEmpty()) {
            ...
            for (int dir = 0; dir < 4; dir++){
                ...
                if (0<=ni && ni<N && 0<= nj && nj <N && visited[ni][nj] == -1){
                    ...
                    // 3. 먹이 후보군 설정
                    // 3-1. 가장 최소거리의 먹이
                    if (0< fishWidth && fishWidth < sharkWidth && visited[ni][nj] <= min_dist) {
                        min_dist = visited[ni][nj];
                        if (ni < targetCoord[0]){
                            targetCoord = new int[] {ni, nj};
                        } else if (ni == targetCoord[0] && nj < targetCoord[1]){
                            targetCoord = new int[] {ni, nj};
                        }
                    }
                }
    ...
    // 4. 하나 먹기
    public void eat(int[] prey){
        sharkStomach++;
        arr[prey[0]][prey[1]] = 0;
        if (sharkStomach == sharkWidth) {
            sharkWidth++;
        }
    }
}
